...
controller.moveSprite(zSprite_Car, 100, 100)
controller.moveSprite(zSprite_Boost_charge_bar, 0, 100)
controller.moveSprite(zSprite_Time, 0, 100)
zSprite_Boost_charge_bar.setFlag(SpriteFlag.GhostThroughWalls, true)
zSprite_Time.setFlag(SpriteFlag.GhostThroughWalls, true)
...
forever(function () {
    if (gameRunning == 1) {
        ...
        zSprite_Car.setVelocity(speed, 0)
        zSprite_Boost_charge_bar.setVelocity(speed, 0)
        zSprite_Time.setVelocity(speed, 0)
        if (xTrail == zSprite_Car.x && controller.right.isPressed()) {
            speed = 0
        }
        ...
        if (speed > maxSpeed == !(controller.A.isPressed())) {
            speed = maxSpeed
        }
        ...
        pause(100)
        zSprite_Time.setText(convertToText(time))
    }
})
